...
## Looping through each mean value (0–6), running 5000 simulations for each mean, calculating the proportion of p-values < 0.05, and plotting power vs true mean to see assocation
```{r}
mu_values = 0:6      
n_sim = 5000        
...
No, the sample average of 𝜇̂ among only the tests where the null was rejected is not approximately equal to the true value of μ, especially when the true mean is small. When μ is close to 0, the test has low power, so the only samples that produce significant results are the ones where the sample mean happened to be much larger than expected by chance. 

As the true value of μ increases, the power of the test increases, and almost all samples lead to rejection.When nearly all samples are significant, the average of 𝜇̂ among rejected samples becomes much closer to both the true μ and the overall sample average.
